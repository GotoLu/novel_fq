...
已发现问题：章节标题容易退化为内部功能标签，例如“对抗”“误判”“翻盘”“低谷”。这类标题能说明章节任务，但缺少读者点击欲、悬疑感和画面钩子，会削弱正文生产阶段的第一眼吸引力。

处理优先级：

1. 标题吸引力属于正文质量闸门，优先级高于文件命名便利。
2. 标题不合格时，先回退到 `plot-architect` 重拟章节卡标题。
3. 标题变更后，`content-writer` 同步正文 H1 和文件名，`progress-tracker` 记录标题优化依据。
4. 同一案件组标题必须风格统一，不能每章各说各话。
...
| 字段 | 要求 |
|------|------|
| 章节编号与标题 | 标题需通过标题闸门：有物件、动作、冲突、异常或悬念；暗示核心事件但不剧透 |
