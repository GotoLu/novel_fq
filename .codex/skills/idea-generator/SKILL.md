...
2. 多维度评分
    ...
    ↓
3. 计算综合得分
    综合得分 = (新颖度×0.2) + (延展性×0.2) + (市场匹配度×0.2) + (冲突强度×0.2) + (不可替代性×0.2)
    ↓
4. 排序筛选
    ↓
5. 返回筛选结果
...
**不可替代性测试（必须执行）**：
- 替换主角测试：换一个普通主角后，故事是否仍然成立？若成立，主角与设定绑定不足。
- 替换设定测试：换掉核心设定后，冲突是否仍然成立？若成立，设定只是皮肤。
- 替换冲突测试：换掉核心冲突后，卖点是否仍然成立？若成立，卖点不够锋利。
- 同类替代测试：读者是否能用一句“这不就是某某类型”概括？若能，必须补强差异化记忆点。
