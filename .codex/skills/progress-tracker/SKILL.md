...
### 3.1 周期质量审计（periodic_audit）

正文连载阶段必须按周期触发以下审计，防止长篇中后期塌陷。

| 周期 | 审计项 | 触发动作 |
|------|--------|----------|
| 每 5-10 章 | 读者反馈闭环、群像轮转表 | 汇总评论高频词、追读/弃读点、角色人气，检查配角私线是否推进 |
| 每 10 章 | 长篇资源账本 | 检查案件、冲突、爽点、钩子、误导是否重复 |
| 每 20-30 章 | 卷级防塌审计 | 检查阶段目标、阶段敌人、阶段谜题、爽点峰值、卷末反转是否仍有效 |
| 每卷结束 | 类型机制复盘 | 检查悬疑公平性、喜剧回收、爽点收益层级和伏笔回收 |
...
**审计规则**：
- 反馈只作为证据，不能被单条评论牵着走。
- 连续两次周期审计出现同类问题，必须进入 `adjust_plan`。
- 任何调整必须记录依据：读者反馈、质量评估、长篇资源账本或卷级防塌卡。
